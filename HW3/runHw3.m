...
signAcademicHonestyPolicy('Varun Sreenivsan', 'vsreenivasan');
...
img_list = {'hough_1', 'hough_2', 'hough_3'};
hough_threshold = [83 58 55];

for i = 1:length(img_list)
    orig_img = imread([img_list{i} '.png']);
    edge_img = imread(['edge_' img_list{i} '.png']);
    hough_img = imread(['accumulator_' img_list{i} '.png']);
    line_img = lineSegmentFinder(orig_img, edge_img, hough_img, hough_threshold(i));
